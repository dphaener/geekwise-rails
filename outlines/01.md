...
Note: The `validates` method is called **class macro**. Fundamentally, it's just a method defined on the ActiveRecord::Base class that, when called, will register a validation rule for the model. We'll see many more class macros moving forward. There are a number of other handy built-in validators as well.
